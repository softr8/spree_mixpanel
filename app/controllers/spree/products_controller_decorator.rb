Spree::ProductsController.class_eval do
  ...
  def mixpanel_track_search
    if params[:keywords]
      MixpanelTracker.new.track(spree_current_user.try(:id), 'Product search', { keywords: params[:keywords] })
    else
  ...
  def mixpanel_track_product_show
    MixpanelTracker.new.track(spree_current_user.try(:id), 'Product Show', {name: @product.name})
  end
end
